  ...
  return (
    <section id="projects" className="w-full py-20 transition-colors duration-300">
      <div className="max-w-[1440px] mx-auto px-8 md:px-16">
        <h2 className="text-4xl font-bold text-gray-900 dark:text-[#aebccf] mb-12">{projectsSection.title}</h2>
            ...
            <div key={index} className="p-4 rounded-lg border border-transparent hover:border-[#3730A3]/30 hover:shadow-md transition-all duration-300">
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {project.tags.map((tag, tagIndex) => (
                  <span key={tagIndex} className="text-[#3730A3] dark:bg-[#383838] dark:text-[#928aff] text-xs font-medium border dark:border[#928aff] border-[#3730A3]/40 px-2.5 py-0.5 rounded-sm">{tag}</span>
                ))}
              </div>
              <div className="flex justify-between items-center text-sm font-medium">
                <a href={project.links.github} target="_blank" rel="noopener noreferrer" className="text-[#3730A3] dark:text-[#e3e3ff] underline">GitHub</a>
                <a href={project.links.live} target="_blank" rel="noopener noreferrer" className="text-[#3730A3] dark:text-[#e3e3ff] underline">View Site</a>
              </div>
            </div>
          ))}
        </div>
